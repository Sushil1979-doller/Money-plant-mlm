...
let isPartner = false;           // true if current user is a partner (i.e. joined via someone else's referral)
...
// FAQ Data: 25 Questions (same as before)
const faqData = [
  {
    en: {
  ...
  // ... (बाकी 24 FAQ items वैसा ही रहेंगे)
];
...
function distributeFunds() {
  // Simulate activation process by hiding the distribute button and showing referral options
  ...
  `;
  // Mark user as activated by paying 27 USDT (simulation)
  isActivated = true;
}
...
function replaceUser() {
  // Check if current user is a partner. Partners cannot use Replace Me.
  if(isPartner){
...
function handleQuit() {
  // Partners are not allowed to use Quit Me
  if(isPartner){
...
function addPartner() {
  // केवल activated user (जो 27 USDT देकर activated हो चुका है) ही पार्टनर जोड़ सकता है
  if(!isActivated) {
    alert("Please activate your account by depositing 27 USDT first!");
    return;
  }
  // यदि current user स्वयं partner है, तो उन्हें नया पार्टनर जोड़ने की अनुमति नहीं होगी
  if(isPartner) {
    alert("You are a Partner and cannot add a new Partner!");
    return;
  }
  // यदि पहले से पार्टनर जोड़ा हुआ है, तो नई एंट्री नहीं होने दे, जब तक कि मौजूदा पार्टनर remove न किया जाए
  if(partnerExists) {
  ...
  // Set the referral start date for the partner (to simulate the 30-day window)
  partnerReferralDate = new Date();
  // Initially, assume partner has 0 referrals
...
function removePartner() {
  // Only activated user (जो Partner जोड़ सकता है) के पास remove करने का ऑप्शन होना चाहिए
  if(!partnerExists) {
    alert("No Partner exists to remove!");
    return;
  }
  // यदि Partner ने अपने रेफरल लिंक से 30 दिन के अंदर कम से कम 2 नए रेफरल जोड़े हैं, तो removal allow नहीं होगा
  ...
  if (confirm("Are you sure you want to remove the current Partner?")) {
    ...
    // Reset the partnerAddress input field so that user can add a new partner
...
/* 
  Note: In an actual implementation, the partnerReferralCount would be updated 
  based on real referral events and the 30-day period checked using partnerReferralDate.
  For simulation purposes, these values may be manually updated or integrated with backend logic.
*/
